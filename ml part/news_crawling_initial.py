...
# install chromium, its driver, and selenium (최초 1회 실행)
...
# target table의 데이터를 불러오는 function
...
# ## 1.2. Insert news into "NEWS" Table
...
# ## 1.3. Settings for Chrome Usage

# In[6]:


# ChromeDriver option 설정 및 ChromeDriver를 return 하는 함수
...
# Crawler Setting
# stdt = "20220101" # fixed (상반기)
# endt = "20220102" # fixed (상반기)
# time_sleep1 = 1   # fixed
# time_sleep2 = 0.5 # fixed
# start_pg = 1      # variant
# end_pg = 2      # variant (네이버뉴스 검색 시 최대 400 page까지 제공)

# Crawling 실행
# test_driver = set_chrome_driver()
# df_test, last_news_id_test = crawler(test_driver, 1, "삼성전자", start_pg, end_pg, stdt, endt, time_sleep1, time_sleep2)
# test_driver.quit()
# df_test_head()


# # 2. All-in-one (최초 1회 실행용 코드)

# In[ ]:


def initial_batch(stdt, endt, time_sleep1=1, time_sleep2=0.5, start_pg=1, end_pg=400):
    ...
    # 3. driver 실행
    initial_batch_driver = set_chrome_driver()

    print("==== INFO: stdt: "+stdt+", endt: "+endt+" ====")

    if (stdt == "20190101"):
        stock_names = stock_names[11:]
    ...
    for stock_name in stock_names:        
        ...
        start = time.time()
        df_news, last_news_id = crawler(initial_batch_driver, last_news_id+1 ,stock_name,
...
if __name__ == "__main__":
    ...
    # 2019년 상반기는 한 번 크롤링 완료
    #stdt = "20190101"
    #endt = "20190630"

    stdts = ["20190701", "20200101", "20200701", "20210101", "20210701", "20220101", "20220701"]
    endts = ["20191231", "20200630", "20201231", "20210630", "20211231", "20220630", "20221115"]
    
    for pair_date in zip(stdts, endts):
        
        initial_batch(pair_date[0], pair_date[1], 1, 0.5, 1, 400)
